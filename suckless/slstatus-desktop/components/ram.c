...
const char *ram_total(const char *unused)
{
    ...
    total =
        total / 1024 / 1024; // Convert to GiB and round down to nearest integer
...
const char *ram_used(const char *unused)
{
    ...
    used = (total - free - buffers - cached) / 1024 /
           1024; // Convert to GiB and round down to nearest integer
...
inline int load_uvmexp(struct uvmexp *uvmexp)
{
...
const char *ram_total(const char *unused)
{
    ...
    return fmt_human(pagetok(uvmexp.npages, uvmexp.pageshift) * 1024, 1024);
}
...
const char *ram_used(const char *unused)
{
    ...
    return fmt_human(pagetok(uvmexp.active, uvmexp.pageshift) * 1024, 1024);
}
...
const char *ram_free(const char *unused)
{
    ...
    if (sysctl(mib, 2, &vm_stats, &len, NULL, 0) < 0 || !len)
        return NULL;
