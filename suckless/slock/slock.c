...
static void drawmessage(Display *dpy, struct lock *lock, const char *message)
{
    ...
    XSetForeground(dpy, lock->gc,
                   lock->colors[INPUT]); /* Use INPUT color for text */
...
static void drawindicator(Display *dpy, struct lock *lock, int len)
{
    ...
    y           = (DisplayHeight(dpy, lock->screen)) / 2 +
...
static void draw_time(Display *dpy, struct lock *lock)
{
    ...
    // Measure text
    XGlyphInfo ext;
    XftTextExtentsUtf8(dpy, timefont, (FcChar8 *)timestr, strlen(timestr),
                       &ext);
...
static void draw_date(Display *dpy, struct lock *lock)
{
    ...
    snprintf(datestr, sizeof(datestr), "%s %d%s, %d", daystr, day, suffix,
             tm_info->tm_year + 1900);

    XGlyphInfo ext;
    XftTextExtentsUtf8(dpy, datefont, (FcChar8 *)datestr, strlen(datestr),
                       &ext);
...
static struct lock *lockscreen(Display *dpy, struct xrandr *rr, int screen)
{
    ...
    lock->xftdraw =
        XftDrawCreate(dpy, lock->win, DefaultVisual(dpy, lock->screen),
