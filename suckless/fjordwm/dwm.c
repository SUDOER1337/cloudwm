 ...
 *
 * fjordwm is designed like any other X client as well. It is
 * driven through handling X events. In contrast to other X clients, a window
 * manager selects for SubstructureRedirectMask on the root window, to receive
 * events about window (dis-)appearance. Only one X connection at a time is
 * allowed to select for this event mask.
 *
 * The event handlers of fjordwm are organized in an array which is accessed
...
static unsigned int pertagtagindex(unsigned int tagset);
...
static const char dwmdir[] = "fjordwm";
...
#define PERTAGTAGMASK ((1U << LENGTH(tags)) - 1)
...
{
    unsigned int i;
    unsigned int normaltags = tagset & PERTAGTAGMASK;

    if (!normaltags || normaltags == PERTAGTAGMASK)
        return 0;
...
static void savepertag(Monitor *m)
{
    ...
    m->pertag->nmasters[curtag]   = m->nmaster;
    m->pertag->mfacts[curtag]     = m->mfact;
    m->pertag->sellts[curtag]     = m->sellt;
    m->pertag->ltidxs[0][curtag]  = m->lt[0];
    m->pertag->ltidxs[1][curtag]  = m->lt[1];
    m->pertag->showbars[curtag]   = m->showbar;
    m->pertag->gappohs[curtag]    = m->gappoh;
    m->pertag->gappovs[curtag]    = m->gappov;
    m->pertag->gappihs[curtag]    = m->gappih;
    m->pertag->gappivs[curtag]    = m->gappiv;
}
...
static void loadpertag(Monitor *m)
{
    ...
    if (!m || !m->pertag)
        return;
    curtag    = m->pertag->curtag;
    m->nmaster = m->pertag->nmasters[curtag];
    m->mfact   = m->pertag->mfacts[curtag];
    m->sellt   = m->pertag->sellts[curtag];
    m->lt[0]   = m->pertag->ltidxs[0][curtag];
    m->lt[1]   = m->pertag->ltidxs[1][curtag];
    m->gappoh  = m->pertag->gappohs[curtag];
    m->gappov  = m->pertag->gappovs[curtag];
    m->gappih  = m->pertag->gappihs[curtag];
    m->gappiv  = m->pertag->gappivs[curtag];
    show       = m->pertag->showbars[curtag];
    if (m->showbar == show)
        return;
    ...
    updatebarpos(m);
    resizebarwin(m);
    if (!showsystray || !systray || m != selmon)
        return;
...
void spawn(const Arg *arg)
{
    if (fork() == 0)
    {
        ...
        fprintf(stderr, "fjordwm: execvp %s failed\n", ((char **)arg->v)[0]);
        perror(" execvp");
        exit(EXIT_FAILURE);
    }
}
...
void cleanupmon(Monitor *mon)
{
    ...
    free(mon->pertag->ltidxs[0]);
    free(mon->pertag->ltidxs[1]);
    free(mon->pertag->showbars);
    free(mon->pertag->enablegaps);
    free(mon->pertag->gappohs);
    free(mon->pertag->gappovs);
    free(mon->pertag->gappihs);
    free(mon->pertag->gappivs);
    free(mon->pertag);
    free(mon);
}
...
Monitor *createmon(void)
{
    ...
    m->pertag->nmasters         = ecalloc(LENGTH(tags) + 1, sizeof(int));
    m->pertag->mfacts           = ecalloc(LENGTH(tags) + 1, sizeof(float));
    m->pertag->sellts           =
        ecalloc(LENGTH(tags) + 1, sizeof(unsigned int));
    m->pertag->ltidxs[0] =
        ecalloc(LENGTH(tags) + 1, sizeof(*m->pertag->ltidxs[0]));
    m->pertag->ltidxs[1] =
        ecalloc(LENGTH(tags) + 1, sizeof(*m->pertag->ltidxs[1]));
    m->pertag->showbars         = ecalloc(LENGTH(tags) + 1, sizeof(int));
    m->pertag->enablegaps       = ecalloc(LENGTH(tags) + 1, sizeof(int));
    m->pertag->gappohs          = ecalloc(LENGTH(tags) + 1, sizeof(int));
    m->pertag->gappovs          = ecalloc(LENGTH(tags) + 1, sizeof(int));
    m->pertag->gappihs          = ecalloc(LENGTH(tags) + 1, sizeof(int));
    m->pertag->gappivs          = ecalloc(LENGTH(tags) + 1, sizeof(int));
    m->pertag->curtag           = 1;
    m->pertag->prevtag          = 1;
    for (i = 0; i <= LENGTH(tags); i++)
    {
        ...
        m->pertag->ltidxs[0][i]   = m->lt[0];
        m->pertag->ltidxs[1][i]   = m->lt[1];
...
int drawstatusbar(Monitor *m, int bh, char *stext)
{
    ...
    while (text[++i])
    {
        if (text[i] == '^' && !isCode)
        {
            ...
            text[i] = '\0';
            w       = TEXTW(text) - lrpad;
            drw_text(drw, x - 2 * sp, vertpadbar / 2, w, bh - vertpadbar, 0,
                     text, 0);
    ...
    {
        w = TEXTW(text) - lrpad;
        drw_text(drw, x - 2 * sp, vertpadbar / 2, w, bh - vertpadbar, 0, text,
                 0);
    }
...
void setlayout(const Arg *arg)
{
    ...
    selmon->pertag->ltidxs[selmon->sellt][selmon->pertag->curtag] =
...
void setup(void)
{
    ...
    XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
                    PropModeReplace, (unsigned char *)"fjordwm", 7);
...
void toggleview(const Arg *arg)
{
    ...
    if (newtagset)
    {
        savepertag(selmon);
        selmon->tagset[selmon->seltags] = newtagset;
        if (newtagset & PERTAGTAGMASK)
        {
            selmon->pertag->prevtag = selmon->pertag->curtag;
            selmon->pertag->curtag  = pertagtagindex(newtagset);
        }
        loadpertag(selmon);
        focus(NULL);
        arrange(selmon);
...
void view(const Arg *arg)
{
    ...
    {
        prevtag                 = selmon->pertag->prevtag;
        selmon->pertag->prevtag = selmon->pertag->curtag;
        selmon->pertag->curtag  = prevtag;
    }
    loadpertag(selmon);
    focus(NULL);
    arrange(selmon);
...
int xerror(Display *dpy, XErrorEvent *ee)
{
    if (ee->error_code == BadWindow ||
        ...
        return 0;
    fprintf(stderr, "fjordwm: fatal error: request code=%d, error code=%d\n",
...
{
    die("fjordwm: another window manager is already running");
    return -1;
}
...
int main(int argc, char *argv[])
{
    ...
    if (!(dpy = XOpenDisplay(NULL)))
        die("fjordwm: cannot open display");
