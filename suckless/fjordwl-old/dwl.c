...
#define VISIBLEON(C, M)         ((M) && (C)->mon == (M) \
		&& ((C)->tags & (M)->tagset[(M)->seltags]) && !(C)->swallowedby)
#define LENGTH(X)               (sizeof X / sizeof X[0])
#define END(A)                  ((A) + LENGTH(A))
#define TAGMASK                 ((1u << TAGCOUNT) - 1)
#define LISTEN(E, L, H)         wl_signal_add((E), ((L)->notify = (H), (L)))
#define LISTEN_STATIC(E, H)     do { struct wl_listener *_l = ecalloc(1, sizeof(*_l)); _l->notify = (H); wl_signal_add((E), _l); } while (0)
#define BORDERPX(C)             (borderpx + ((C)->swallowing \
		? (int)ceilf(swallowborder * (C)->swallowing->bw) : 0))
...
typedef struct DwlIpcOutput DwlIpcOutput;
...
static void dwl_ipc_manager_bind(struct wl_client *client, void *data, uint32_t version,
		uint32_t id);
...
static void dwl_ipc_output_release(struct wl_client *client,
		struct wl_resource *resource);
static int ensurestatuspath(void);
static void fprintjsonchars(FILE *f, const char *s);
...
static struct wlr_output_mode *outputmodematch(struct wlr_output *wlr_output,
...
static void setlayoutmon(Monitor *m, uint32_t index);
static void setmfact(const Arg *arg);
static void setmon(Client *c, Monitor *m, uint32_t newtags);
static void setmonview(Monitor *m, uint32_t tagmask, int toggle_tagset);
static void setselectedmon(Monitor *m);
...
static char waybar_status_dir[PATH_MAX];
static char waybar_window_path[PATH_MAX];
...
struct Pertag {
	...
	int nmasters[TAGCOUNT + 1];
	float mfacts[TAGCOUNT + 1];
	unsigned int sellts[TAGCOUNT + 1];
	const Layout *ltidxs[TAGCOUNT + 1][2];
	int enablegaps[TAGCOUNT + 1];
	int gappihs[TAGCOUNT + 1];
	int gappivs[TAGCOUNT + 1];
	int gappohs[TAGCOUNT + 1];
	int gappovs[TAGCOUNT + 1];
};
...
centerfloating(Client *c)
{
	...
	geo = c->geom;
	geo.x = c->mon->w.x + (c->mon->w.width - geo.width) / 2;
	geo.y = c->mon->w.y + (c->mon->w.height - geo.height) / 2;
	resize(c, geo, 0);
}
...
applyrules(Client *c)
{
	...
	if (enableautoswallow && !c->noswallow && !c->isfloating
		...
		swallow_parent = termforwin(c);
		if (swallow_parent) {
			mon = swallow_parent->mon;
			newtags = swallow_parent->tags;
			c->isfloating = swallow_parent->isfloating;
		}
	}
	setmon(c, mon, newtags);
	if (swallow_parent)
		swallow(c, swallow_parent);
}
...
outputmodematch(struct wlr_output *wlr_output, int width, int height, int refresh)
{
	struct wlr_output_mode *best = NULL, *mode;
	int best_delta = INT_MAX;
	int requested_refresh = refresh > 0 ? refresh * 1000 : 0;
	...
	wl_list_for_each(mode, &wlr_output->modes, link) {
		...
		delta = abs(mode->refresh - requested_refresh);
	...
	if (requested_refresh > 0 && best_delta > 1000)
		return NULL;
...
createmon(struct wl_listener *listener, void *data)
{
	...
	const MonitorRule *matched_rule = NULL, *r;
	...
	for (r = monrules; r < END(monrules); r++) {
		if (!r->name || strstr(wlr_output->name, r->name)) {
			...
			mode = outputmodematch(wlr_output, r->mode_width, r->mode_height,
					r->mode_refresh);
			break;
		}
	}
	...
	/* Fall back to the preferred mode if the configured one is unavailable. */
	wlr_output_state_set_mode(&state, mode ? mode : wlr_output_preferred_mode(wlr_output));
	if (matched_rule && matched_rule->mode_width > 0 && matched_rule->mode_height > 0 && !mode)
		wlr_log(WLR_INFO, "output %s does not support %dx%d@%d, using preferred mode",
				wlr_output->name, matched_rule->mode_width,
				matched_rule->mode_height, matched_rule->mode_refresh);
		...
		uint32_t id)
{
	...
	(void)data;
	manager_resource = wl_resource_create(client, &zdwl_ipc_manager_v2_interface,
			version, id);
	if (!manager_resource) {
		...
		uint32_t id, struct wl_resource *output)
{
	...
	m = wlr_output ? wlr_output->data : NULL;
	if (!m)
		return;
...
void
dwl_ipc_output_printstatus(Monitor *m)
{
	DwlIpcOutput *ipc_output;
...
dwl_ipc_output_printstatus_to(DwlIpcOutput *ipc_output)
{
	...
	size_t tag;
	unsigned int clients_on_tag, focused_client, layout_index;
	...
	for (tag = 0; tag < TAGCOUNT; tag++) {
		...
		zdwl_ipc_output_v2_send_tag(ipc_output->resource, tag, state,
				clients_on_tag, focused_client);
	}

	if (focused) {
		title = client_get_title(focused);
		appid = client_get_appid(focused);
		if (!title || !title[0] || !strcmp(title, "broken"))
			title = (!appid || !strcmp(appid, "broken")) ? "" : appid;
		if (!appid || !strcmp(appid, "broken"))
			appid = "";
	}

	layout_index = (unsigned int)(m->lt[m->sellt] - layouts);
	zdwl_ipc_output_v2_send_layout(ipc_output->resource, layout_index);
		...
		uint32_t and_tags, uint32_t xor_tags)
{
	...
	newtags = ((c->tags & and_tags) ^ xor_tags) & TAGMASK;
	if (!newtags)
		return;
		...
		uint32_t index)
{
	...
	setlayoutmon(ipc_output->mon, index);
}
		...
		uint32_t tagmask, uint32_t toggle_tagset)
{
	...
	setmonview(ipc_output->mon, tagmask, toggle_tagset);
}
...
static int
ensurestatuspath(void)
{
	const char *runtime_dir;
	int n;

	if (waybar_window_path[0])
		return 0;

	runtime_dir = getenv("XDG_RUNTIME_DIR");
	if (runtime_dir && runtime_dir[0]) {
		n = snprintf(waybar_status_dir, sizeof(waybar_status_dir),
				"%s/fjordwl", runtime_dir);
	} else {
		n = snprintf(waybar_status_dir, sizeof(waybar_status_dir),
				"/tmp/fjordwl-%u", getuid());
	}
	if (n < 0 || (size_t)n >= sizeof(waybar_status_dir))
		return -1;

	if (mkdir(waybar_status_dir, 0700) < 0 && errno != EEXIST)
		return -1;

	n = snprintf(waybar_window_path, sizeof(waybar_window_path),
			"%s/waybar-window.json", waybar_status_dir);
	if (n < 0 || (size_t)n >= sizeof(waybar_window_path)) {
		waybar_window_path[0] = '\0';
		return -1;
	}
...
static void
fprintjsonchars(FILE *f, const char *s)
{
...
static void
fprintjsonstring(FILE *f, const char *s)
{
	fputc('"', f);
	fprintjsonchars(f, s);
	fputc('"', f);
...
focusortogglescratch(const Arg *arg)
{
	...
	wl_list_for_each(c, &clients, link) {
		if (c->scratchkey == ((const char **)arg->v)[0][0]) {
			found = 1;
			break;
		}
	}
...
monocle(Monitor *m)
{
	Client *c;
	int n = 0;
	int oe = m->pertag && m->pertag->enablegaps[m->pertag->curtag];
	...
	wl_list_for_each(c, &clients, link) {
		...
		if (!monoclegaps || !oe) {
			...
			resize(c, (struct wlr_box){
				.x = m->w.x + m->gappov,
				.y = m->w.y + m->gappoh,
				.width = m->w.width - 2 * m->gappov,
				.height = m->w.height - 2 * m->gappoh,
			}, 0);
		}
		n++;
	}
		...
		double dx_unaccel, double dy_unaccel)
{
	...
	if (time && sloppyfocus && selmon != oldselmon
			&& (!c || client_is_unmanaged(c) || !surface))
		printstatus();
}
...
printstatus(void)
{
	...
	if (ensurestatuspath() < 0)
		return;

	n = snprintf(tmp_path, sizeof(tmp_path), "%s.tmp", waybar_window_path);
	if (n < 0 || (size_t)n >= sizeof(tmp_path))
		return;

	c = selmon ? focustop(selmon) : NULL;
	if (c) {
		text = client_get_title(c);
		appid = client_get_appid(c);
		if (!text || !text[0] || !strcmp(text, "broken"))
			text = (!appid || !strcmp(appid, "broken")) ? "" : appid;
		if (!appid || !strcmp(appid, "broken"))
			appid = "";
		class = c->isfullscreen ? "fullscreen"
	...
	fputs("{\"text\":", f);
	fprintjsonstring(f, text);
	fputs(",\"tooltip\":", f);
	if (appid[0] && strcmp(appid, text)) {
		fputc('"', f);
		fprintjsonchars(f, appid);
		fputs("\\n", f);
		fprintjsonchars(f, text);
		fputc('"', f);
	} else {
		fprintjsonstring(f, text);
	}
	fputs(",\"class\":", f);
	fprintjsonstring(f, class);
	fputs("}\n", f);
	...
	if (rename(tmp_path, waybar_window_path) < 0)
		unlink(tmp_path);
}
...
run(char *startup_cmd)
{
	...
	setselectedmon(xytomon(cursor->x, cursor->y));
...
setgaps(int oh, int ov, int ih, int iv)
{
	...
	selmon->pertag->gappohs[selmon->pertag->curtag] = selmon->gappoh;
	selmon->pertag->gappovs[selmon->pertag->curtag] = selmon->gappov;
	selmon->pertag->gappihs[selmon->pertag->curtag] = selmon->gappih;
	selmon->pertag->gappivs[selmon->pertag->curtag] = selmon->gappiv;
	arrange(selmon);
}
...
void
setlayoutmon(Monitor *m, uint32_t index)
{
	if (!m || index >= LENGTH(layouts) || &layouts[index] == m->lt[m->sellt])
		return;
...
void
setmonview(Monitor *m, uint32_t tagmask, int toggle_tagset)
{
	size_t i;
	...
	if (!m || !newtags)
		return;
	if (!toggle_tagset && newtags == m->tagset[m->seltags])
		return;
	if (toggle_tagset)
		m->seltags ^= 1;
	...
	if (newtags == TAGMASK) {
		...
		for (i = 0; !(newtags & (1u << i)); i++)
			;
		m->pertag->curtag = (unsigned int)i + 1;
	}
	...
	m->gappih = m->pertag->gappihs[m->pertag->curtag];
	m->gappiv = m->pertag->gappivs[m->pertag->curtag];
	m->gappoh = m->pertag->gappohs[m->pertag->curtag];
	m->gappov = m->pertag->gappovs[m->pertag->curtag];
...
spawnscratch(const Arg *arg)
{
	if (fork() == 0) {
		...
		die("fjordwl: execvp %s failed:", ((char **)arg->v)[1]);
	}
}
...
spawn(const Arg *arg)
{
	if (fork() == 0) {
		...
		die("fjordwl: execvp %s failed:", ((char **)arg->v)[0]);
	}
}
...
termforwin(Client *c)
{
	...
	for (i = 0; i < pids_len; i++) {
		wl_list_for_each(p, &clients, link) {
			if (!p->pid || !p->isterm || p->noswallow || p->swallowedby)
				continue;
			if (pids[i] == p->pid)
				return p;
		}
...
tile(Monitor *m)
{
	unsigned int mw, my, ty, h, r;
	int n = 0;
	int oe = m->pertag ? m->pertag->enablegaps[m->pertag->curtag] : 1;
	int ie = oe;
	int i;
	Client *c;
	...
	if (n > m->nmaster)
		mw = m->nmaster ? (unsigned int)roundf((m->w.width + m->gappiv * ie) * m->mfact) : 0;
	else
...
toggleview(const Arg *arg)
{
	...
	if (newtagset != TAGMASK
		...
		selmon->pertag->prevtag = selmon->pertag->curtag;
		for (i = 0; !(newtagset & (1u << i)); i++)
			;
		selmon->pertag->curtag = (unsigned int)i + 1;
	}
...
createnotifyx11(struct wl_listener *listener, void *data)
{
	...
	c->type = X11;
	c->bw = client_is_unmanaged(c) ? 0 : BORDERPX(c);
