...
#define ROWS 16   // time steps (vertical)
#define COLS 5    // tracks/channels (horizontal)
...
typedef struct {
    ...
    int active;
    Mix_Chunk* sound;
    int recent_index; // 0 if no recent, 1-9 for assigned
    int playing;      // 1 if currently playing this step
} Cell;

Cell tracker[ROWS][COLS];
int cur_row = 0, cur_col = 0;
int running = 1; // sequencer running flag

char recent_sounds[RECENT_MAX][WAV_NAME_LEN];
int recent_count = 0;
int current_recent = 0; // last selected recent
...
void draw_grid(int step) {
    ...
    int start_y = (LINES - (grid_height + 6))/2; // +6 for header/status
    int start_x = (COLS*6 < COLS) ? 0 : (COLS*6 - grid_width)/2;

    // App title
    mvprintw(start_y-2, start_x, " [ Vertical TUI Tracker ] ");

    for(int r=0;r<ROWS;r++) {
        for(int c=0;c<COLS;c++) {
            int y = start_y + r;
            int x = start_x + c*6;
            if(r==cur_row && c==cur_col) attron(A_REVERSE);
            if(r==step) attron(A_BOLD); // highlight current playing step
            mvprintw(y, x, tracker[r][c].active ? "[%d]" : "[ ]", tracker[r][c].recent_index);
            if(r==step) attroff(A_BOLD);
            if(r==cur_row && c==cur_col) attroff(A_REVERSE);
        }
    }
...
int file_browser(char* out_path, size_t max_len, const char* start_dir) {
    ...
    while(1) {
        ...
        int start_y = (LINES - (valid_count + 4))/2;
        int start_x = 4;

        clear();
        mvprintw(start_y, start_x, "Browsing: %s", current_dir);
        mvprintw(start_y+1, start_x, "Search: %s%s", search, search_mode?"_":"");
        for(int i=0;i<valid_count;i++) {
            if(i==selection) attron(A_REVERSE);
            mvprintw(start_y+2+i, start_x, "%s", entries[i]);
            if(i==selection) attroff(A_REVERSE);
        }
        mvprintw(LINES-1,0,"hjkl: nav | /: search | enter: select | backspace: up | q: quit");
        refresh();
...
int main() {
    ...
    while(1) {
        ...
        if(ch!=ERR) {
            switch(ch) {
                ...
                case 'i': {
                    ...
                    echo();
                    if(file_browser(path,sizeof(path),".")) insert_wav(path);
        ...
        if(running) {
            play_step(step);
            step=(step+1)%ROWS; // vertical steps
        }
