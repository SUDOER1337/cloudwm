...
void draw_grid(int step) {
    ...
    for(int r=0;r<ROWS;r++) {
        for(int c=0;c<COLS;c++) {
            ...
            // Step row highlight
            if(r == step) attron(COLOR_PAIR(4)); // blue for playing row

            // Cursor highlight overrides row
            if(r==cur_row && c==cur_col) attron(A_REVERSE);

            // Active cells
            if(tracker[r][c].active) attron(COLOR_PAIR(3));

            // Individual playing cell bold
            if(tracker[r][c].playing) attron(A_BOLD);
...
int file_browser(char* out_path, size_t max_len, const char* start_dir) {
    ...
    while(1) {
        ...
        int ch = getch();
    if(search_mode) {
        if(ch == '\n' || ch == 27) { // Enter or Esc exits search mode
        search_mode = 0;
...
int main() {
    ...
    if(has_colors()) {
        ...
        init_pair(4, COLOR_WHITE, COLOR_BLUE);    // currently playing row
    }
