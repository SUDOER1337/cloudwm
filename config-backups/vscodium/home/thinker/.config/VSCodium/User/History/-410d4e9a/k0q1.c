...
#define ROWS 16   // steps
#define COLS 5    // tracks/channels
...
typedef struct {
    ...
    int active;
    Mix_Chunk* sound;
    int recent_index; // 0 if no recent, 1-9
...
void draw_grid(int step) {
    ...
    int start_y = (LINES - (grid_height + 6))/2; 
...
int file_browser(char* out_path, size_t max_len, const char* start_dir) {
    ...
    while(1) {
        ...
        int start_x = (COLS*6 < COLS) ? 0 : (COLS*6 - valid_count*6)/2;
        clear();
        ...
        if(search_mode) {
            ...
            if(ch=='q') return 0; // only quit when not searching
            switch(ch) {
