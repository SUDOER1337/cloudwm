...
int center_x(int text_len) {
    int x = (COLS > text_len ? 0 : (COLS*6 - text_len)/2);
    if(x < 0) x = 0;
    return (COLS*6 < text_len) ? 0 : (COLS*6 - text_len)/2;
}


int center_x_text(int text_len) {
    int x = (COLS*6 < text_len) ? 0 : (COLS*6 - text_len)/2;
    return x;
}
