...
(function (DisplayOption) {
    DisplayOption[DisplayOption["Nothing"] = 'Nothing'] = "Nothing";
    DisplayOption[DisplayOption["EmptyLine"] = 'EmptyLine'] = "EmptyLine";
    DisplayOption[DisplayOption["GroupName"] = 'GroupName'] = "GroupName";
})(DisplayOption || (DisplayOption = {}));
class CssOrganizer {
    constructor(context) {
        this.userGroupSorter = null;
        this.currentGroupSorter = null;
    ...
    info(msg, ...items) { vscode.window.showInformationMessage(msg, ...items); }
    warning(msg, ...items) { vscode.window.showWarningMessage(msg, ...items); }
    error(msg, ...items) { vscode.window.showErrorMessage(msg, ...items); }
    ...
    getDisplayOption() {
        const displayOption = vscode.workspace.getConfiguration().get('css-organizer.displayOption');
        return displayOption;
    }
    getUserDefinedGroupsFile() {
        return vscode.workspace.getConfiguration().get('css-organizer.userGroupingFile');
    }
    getUseUserDefinedGroups() {
        return vscode.workspace.getConfiguration().get('css-organizer.userGroupingFileUse');
    }
    organizeGrouped() {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
        ...
        if (ast) {
            ...
            rules.forEach((r) => r.sortFrom(this.currentGroupSorter, displayOption));
            editor.edit((edit) => {
                rules.reverse().forEach((r, i, arr) => {
                    edit.replace(r.getReplaceRange(), i === arr.length - 1 ? r.groupSortedSrc.trimEnd() : r.groupSortedSrc);
                });
            });
        }
    }
    loadGroupSorters(context) {
        ...
        if (this.getUseUserDefinedGroups()) {
            const userJsonPath = path.join(context.extensionPath, 'resources', this.getUserDefinedGroupsFile());
...
exports.CssOrganizer = CssOrganizer;
class AST {
    constructor(ast, doc) {
        this.astData = ast;
        this.doc = doc;
        this.rules = this.astData.stylesheet.rules.filter((r) => r.type === 'rule' || AST.RULE_CONTAINING_TYPES.includes(r.type))
            .map((r) => {
            if (AST.RULE_CONTAINING_TYPES.includes(r.type)) {
                return r.rules.map((rr) => new Rule(rr));
            }
            return new Rule(r);
        });
    }
    ...
    pickRuleAt(pos) {
        const found = this.rules.find(r => r.range.contains(pos));
        found?.setSourceFrom(this.doc);
        return [found];
    }
    pickRulesFrom(selection) {
        if (selection.isEmpty) {
            const found = this.rules.find(r => r.range.contains(selection.active));
            found?.setSourceFrom(this.doc);
            return [found];
        }
...
AST.RULE_CONTAINING_TYPES = ['document', 'host', 'media', 'supports'];
class Rule {
    constructor(ruleObject) {
        ...
        if (this.ruleObject.declarations) {
            this.decs = this.ruleObject.declarations.map((d) => new Declaration(d));
        }
    }
    setSourceFrom(doc) {
    ...
    sortFrom(groupSorter, displayOption) {
        if (!groupSorter)
            return;
        groupSorter.clear();
    ...
    getReplaceRange() {
        ...
        return new vscode.Range(new vscode.Position(minLineNr, 0), new vscode.Position(maxLineNr, maxLineCharNr));
    }
    getAlphaSortedSource(groupTitles) {
        const decsSrc = this.decs.reduce((p, c) => {
            if (groupTitles.includes(c.decObject.comment)) {
                return p;
            }
            return p + c.src + '\n';
        }, '').trimEnd();
...
class GroupSorter {
    ...
    put(dec) {
        ...
        if (this.groupTitles.includes(dec.decObject.comment)) {
            return;
        }
        group.put(dec);
    }
...
class Group {
    constructor(jsonObject) {
        ...
        if (jsonObject.properties) {
            jsonObject.properties.forEach((p) => this.propertyNames.push(p));
        }
    }
    put(dec) {
    ...
    sort() {
        ...
        this.propertyDeclarations.sort((a, b) => {
    ...
    getSource(displayOption) {
        if (this.propertyDeclarations.length === 0) {
            return '';
        }
        let decsSrc = '';
        if (displayOption === DisplayOption.GroupName) {
            decsSrc += '/*' + this.title + '*/\n';
        }
        if (displayOption === DisplayOption.EmptyLine) {
            decsSrc += '\n';
        }
...
//# sourceMappingURL=css-organizer.js.map